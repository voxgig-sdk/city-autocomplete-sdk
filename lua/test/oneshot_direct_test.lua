...
describe("OneshotDirect", function()
  it("should direct-list-oneshot", function()
    local setup = oneshot_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-oneshot", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
    if setup.live then
      for _, _live_key in ipairs({"city_name01", "country01", "language01"}) do
...
function oneshot_direct_setup(mockres)
  runner.load_env_local()

  local calls = {}

  local env = runner.env_override({
    ["CITYAUTOCOMPLETE_TEST_ONESHOT_ENTID"] = {},
