-- RegionTranslationDto direct test
...
describe("RegionTranslationDtoDirect", function()
  it("should direct-list-region_translation_dto", function()
    local setup = region_translation_dto_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-region_translation_dto", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
    if setup.live then
      for _, _live_key in ipairs({"region_translation_dto01"}) do
...
function region_translation_dto_direct_setup(mockres)
