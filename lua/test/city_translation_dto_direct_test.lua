-- CityTranslationDto direct test
...
describe("CityTranslationDtoDirect", function()
  it("should direct-list-city_translation_dto", function()
    local setup = city_translation_dto_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-city_translation_dto", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
    if setup.live then
      for _, _live_key in ipairs({"city_translation_dto01"}) do
...
function city_translation_dto_direct_setup(mockres)
