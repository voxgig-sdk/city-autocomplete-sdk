...
describe("SettlementTypeDirect", function()
  it("should direct-list-settlement_type", function()
    local setup = settlement_type_direct_setup({
      { id = "direct01" },
      { id = "direct02" },
    })
    local _should_skip, _reason = runner.is_control_skipped("direct", "direct-list-settlement_type", setup.live and "live" or "unit")
    if _should_skip then
      pending(_reason or "skipped via sdk-test-control.json")
      return
    end
    if setup.live then
      for _, _live_key in ipairs({"city01"}) do
...
function settlement_type_direct_setup(mockres)
