...
local DistanceEntity = {}
DistanceEntity.__index = DistanceEntity


function DistanceEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, DistanceEntity)
  self._name = "distance"
  self._client = client
...
function DistanceEntity:get_name()
  return self._name
end


function DistanceEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return DistanceEntity.new(self._client, opts)
end
...
function DistanceEntity:mark_deleted()
  self._deleted = true
end


function DistanceEntity:deleted()
  return true == self._deleted
end


function DistanceEntity:data_set(args)
  if args ~= nil then
...
function DistanceEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function DistanceEntity:match_set(args)
  if args ~= nil then
...
function DistanceEntity:match_get()
...
function DistanceEntity:stream(action, args, callopts)
...
---@param reqmatch DistanceLoadMatch
---@param ctrl? table
---@return Distance
---@return string? err
function DistanceEntity:load(reqmatch, ctrl)
  local utility = self._utility
...
function DistanceEntity:_run_op(ctx, post_done)
  local utility = self._utility
