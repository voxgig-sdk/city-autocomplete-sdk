...
local CityDtoEntity = {}
CityDtoEntity.__index = CityDtoEntity


function CityDtoEntity.new(client, entopts)
  entopts = entopts or {}
  ...
  local self = setmetatable({}, CityDtoEntity)
  self._name = "city_dto"
  self._client = client
...
function CityDtoEntity:get_name()
  return self._name
end


function CityDtoEntity:make()
  local opts = {}
  for k, v in pairs(self._entopts) do
    opts[k] = v
  end
  return CityDtoEntity.new(self._client, opts)
end
...
function CityDtoEntity:mark_deleted()
  self._deleted = true
end


function CityDtoEntity:deleted()
  return true == self._deleted
end


function CityDtoEntity:data_set(args)
  if args ~= nil then
...
function CityDtoEntity:data_get()
  self._utility.feature_hook(self._entctx, "GetData")
  return vs.clone(self._data)
end


function CityDtoEntity:match_set(args)
  if args ~= nil then
...
function CityDtoEntity:match_get()
...
function CityDtoEntity:stream(action, args, callopts)
...
---@param reqmatch CityDtoListMatch
---@param ctrl? table
---@return CityDto[]
---@return string? err
function CityDtoEntity:list(reqmatch, ctrl)
  local utility = self._utility
...
function CityDtoEntity:_run_op(ctx, post_done)
  local utility = self._utility
