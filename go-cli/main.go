// Command CityAutocomplete-cli is a CLI + REPL for the CityAutocomplete SDK,
...
const prompt = "city-autocomplete"

// entitiesHelp is the space-separated entity list shown by /help.
const entitiesHelp = "city city_dto city_translation_dto country country_translation_dto distance language oneshot region region_translation_dto settlement_type"
...
func usage(out io.Writer) {
	...
	fmt.Fprintln(out, "Env:      CITY_AUTOCOMPLETE_APIKEY (api key), CITY_AUTOCOMPLETE_BASE (base url override)")
...
func run(args []string, in io.Reader, out, errOut io.Writer) int {
	...
	// Configure from the environment: CITY_AUTOCOMPLETE_APIKEY carries the API key and
	// CITY_AUTOCOMPLETE_BASE optionally overrides the API base URL (e.g. production).
	// Both injectable by a secrets vault. Unset -> nil config defaults.
	var opts map[string]any
	if apikey := os.Getenv("CITY_AUTOCOMPLETE_APIKEY"); apikey != "" {
		opts = map[string]any{"apikey": apikey}
	}
	if base := os.Getenv("CITY_AUTOCOMPLETE_BASE"); base != "" {
		if opts == nil {
			opts = map[string]any{}
		}
		opts["base"] = base
	}
	client := sdk.NewCityAutocompleteSDK(opts)
