# city-autocomplete-cli

boru-driven command-line client **and** interactive REPL for the CityAutocomplete
...
```sh
# 1. Build a native binary (-> dist/<os>-<arch>/city-autocomplete-cli)
make build

# 2. See usage (words, entities, env vars)
./city-autocomplete-cli --help

# 3. Provide credentials once, via the environment
export CITY_AUTOCOMPLETE_APIKEY=sk_live_xxx

# 4. Each command line is ONE boru expression, run against the API:
./city-autocomplete-cli load 1 city            # {id:1} shorthand
./city-autocomplete-cli load '{id:1}' city       # explicit match map

# 5. Override the API base URL for a single call
CITY_AUTOCOMPLETE_BASE=https://api.example.com ./city-autocomplete-cli load 1 city

# 6. No arguments -> interactive REPL
./city-autocomplete-cli
city-autocomplete> load 1 city
city-autocomplete> /quit
```
...
1. **Build the binary.** From this `go-cli/` directory:
   ...
   ```sh
   make build          # -> dist/<os>-<arch>/city-autocomplete-cli
   ```
...
3. **Run a query.** Evaluate an boru expression against the API (or run with no
   ...
   ```sh
   ./dist/*/city-autocomplete-cli load 1 city
   ```
...
```sh
./city-autocomplete-cli load 1 city          # scalar shorthand for {id:1}
./city-autocomplete-cli load '{id:1}' city     # explicit match map
```
...
./city-autocomplete-cli load 1 city
```
...
Run with no arguments to open a REPL (prompt `city-autocomplete>`). Each line is
...
| Target | Result |
|--------|--------|
| `make build` | Native binary at `dist/<os>-<arch>/city-autocomplete-cli`. |
...
(`./city-autocomplete-cli <expr>`) and interactively (the REPL), and expressions compose the
