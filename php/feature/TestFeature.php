...
// CityAutocomplete SDK test feature

require_once __DIR__ . '/BaseFeature.php';
require_once __DIR__ . '/../utility/Param.php';

class CityAutocompleteTestFeature extends CityAutocompleteBaseFeature
{
    ...
    public function init(CityAutocompleteContext $ctx, array $options): void
    {
        ...
        $test_fetcher = function (CityAutocompleteContext $fctx, string $_fullurl, array $_fetchdef) use ($entity): array {
            ...
            if ($op->name === 'load') {
                ...
                $id = CityAutocompleteParam::call($fctx, 'id');
    ...
    public function makeNetsim(array $net, callable $inner): callable
    {
        ...
        return function (CityAutocompleteContext $fctx, string $url, array $fetchdef) use ($net, $inner, $pick_latency, $sleep): array {
            $this->_netcalls++;
            $call = $this->_netcalls;
     ...
     */
    public function buildArgs(CityAutocompleteContext $ctx, $op, $args): array
    {
        ...
        foreach ($keys as $k) {
            ...
            if ($is_id || $in_reqd) {
                $v = CityAutocompleteParam::call($ctx, $k);
