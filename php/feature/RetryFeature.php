...
// CityAutocomplete SDK retry feature
...
class CityAutocompleteRetryFeature extends CityAutocompleteBaseFeature
{
    ...
    public function init(CityAutocompleteContext $ctx, array $options): void
    {
        ...
        $utility->fetcher = function (CityAutocompleteContext $ctx2, string $url, array $fetchdef) use ($inner): array {
            return $this->_with_retry($ctx2, $url, $fetchdef, $inner);
        };
    }

    private function _with_retry(CityAutocompleteContext $ctx, string $url, array $fetchdef, callable $inner): array
    {
        $opts = $this->options ?? [];
    ...
    private function _track(CityAutocompleteContext $_ctx, int $attempt, mixed $res, mixed $err, float $wait): void
    {
        $client = $this->client;
