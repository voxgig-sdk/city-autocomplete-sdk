...
class CityTranslationDtoDirectTest extends TestCase
{
    public function test_direct_list_city_translation_dto(): void
    {
        $setup = city_translation_dto_direct_setup([
            ["id" => "direct01"],
            ["id" => "direct02"],
        ]);
        [$_shouldSkip, $_reason] = Runner::is_control_skipped("direct", "direct-list-city_translation_dto", $setup["live"] ? "live" : "unit");
        if ($_shouldSkip) {
            $this->markTestSkipped($_reason ?? "skipped via sdk-test-control.json");
            return;
        }
        if ($setup["live"]) {
            foreach (["city_translation_dto01"] as $_liveKey) {
