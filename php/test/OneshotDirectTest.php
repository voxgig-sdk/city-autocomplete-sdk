...
class OneshotDirectTest extends TestCase
{
    public function test_direct_list_oneshot(): void
    {
        $setup = oneshot_direct_setup([
            ["id" => "direct01"],
            ["id" => "direct02"],
        ]);
        [$_shouldSkip, $_reason] = Runner::is_control_skipped("direct", "direct-list-oneshot", $setup["live"] ? "live" : "unit");
        if ($_shouldSkip) {
            $this->markTestSkipped($_reason ?? "skipped via sdk-test-control.json");
            return;
        }
        if ($setup["live"]) {
            foreach (["city_name01", "country01", "language01"] as $_liveKey) {
