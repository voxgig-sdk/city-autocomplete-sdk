...
// CityAutocomplete SDK feature test
//
...
class FtCtrl extends CityAutocompleteControl
{
...
class FtClient
{
    ...
    private CityAutocompleteUtility $utility;

    public function __construct(CityAutocompleteUtility $utility, array $options)
    {
    ...
    public function get_utility(): CityAutocompleteUtility
    {
        return $this->utility;
    }
}
...
class FtRecorder
{
    ...
    {
        return function (CityAutocompleteContext $_ctx, string $url, array $fetchdef): array {
...
class FtHarness
{
    ...
    public CityAutocompleteContext $rootctx;
    ...
    public function __construct(
        ...
        $utility->fetcher = $server ?? function (CityAutocompleteContext $_ctx, string $_url, array $fetchdef): array {
        ...
        foreach ($features as $fspec) {
            ...
            $f = CityAutocompleteConfig::make_feature($name);
    ...
    private static function buildUrl(CityAutocompleteSpec $spec): string
    {
        $keys = [];
    ...
    public function op(array $o = []): array
    {
        ...
        if (($ctx->out['point'] ?? null) instanceof CityAutocompleteError) {
        ...
        if (is_array($fetched)) {
            $ctx->response = new CityAutocompleteResponse($fetched);
        }
    ...
    private function fail(CityAutocompleteContext $ctx, mixed $err): array
    {
...
class FeatureTest extends TestCase
{
    ...
    public function test_retry_retries_a_transport_error_until_exhausted(): void
    {
        ...
        $clock = new FtClock();
        $n = 0;
        $server = function (CityAutocompleteContext $ctx, string $_url, array $_fd) use (&$n): array {
            $n++;
