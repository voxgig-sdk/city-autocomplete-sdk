...
class SettlementTypeDirectTest extends TestCase
{
    public function test_direct_list_settlement_type(): void
    {
        $setup = settlement_type_direct_setup([
            ["id" => "direct01"],
            ["id" => "direct02"],
        ]);
        [$_shouldSkip, $_reason] = Runner::is_control_skipped("direct", "direct-list-settlement_type", $setup["live"] ? "live" : "unit");
        if ($_shouldSkip) {
            $this->markTestSkipped($_reason ?? "skipped via sdk-test-control.json");
            return;
        }
        if ($setup["live"]) {
            foreach (["city01"] as $_liveKey) {
