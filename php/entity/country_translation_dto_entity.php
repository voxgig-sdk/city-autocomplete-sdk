...
// CityAutocomplete SDK CountryTranslationDto entity
...
class CountryTranslationDtoEntity
{
    ...
    public function __construct($client, ?array $entopts = null)
    {
        ...
        $this->_name = "country_translation_dto";
        $this->_client = $client;
    ...
    {
        $opts = $this->_entopts;
        return new CountryTranslationDtoEntity($this->_client, $opts);
    }

    /**
     * @param CountryTranslationDto|array $args CountryTranslationDto data (assoc-array) to store.
     */
    public function data_set($args): void
    {
        if ($args) {
            $this->_data = CityAutocompleteHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetData");
        }
    }

    /**
     * @return CountryTranslationDto|array The current CountryTranslationDto data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of CountryTranslationDto fields).
     */
    public function match_set($args): void
    {
        if ($args) {
            $this->_match = CityAutocompleteHelpers::to_map(Struct::clone($args)) ?? [];
            ($this->_utility->feature_hook)($this->_entctx, "SetMatch");
        }
    }

    /**
     * @return array The current match filter (any subset of CountryTranslationDto fields).
     */
    ...
    /**
     * List CountryTranslationDto items matching the given filter.
     *
     * @param CountryTranslationDtoListMatch|array|null $reqmatch Match filter (any subset
     *   of CountryTranslationDto fields) as an assoc-array; CountryTranslationDtoListMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return CountryTranslationDto[]|array A list of CountryTranslationDto items as assoc-arrays at
     *   the SDK boundary; throws CityAutocompleteError on failure (item-5 convention).
     */
