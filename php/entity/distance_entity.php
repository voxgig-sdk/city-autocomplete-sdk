...
// CityAutocomplete SDK Distance entity
...
class DistanceEntity
{
    ...
    {
        $opts = $this->_entopts;
        return new DistanceEntity($this->_client, $opts);
    }

    /**
     * @param Distance|array $args Distance data (assoc-array) to store.
     */
    ...
    /**
     * @return Distance|array The current Distance data as an assoc-array.
     */
    ...
    /**
     * @param array $args Match filter (any subset of Distance fields).
     */
    ...
    /**
     * @return array The current match filter (any subset of Distance fields).
     */
    ...
    /**
     * Load a single Distance.
     *
     * @param DistanceLoadMatch|array|null $reqmatch Match criteria (id/query
     *   fields) as an assoc-array; a typed DistanceLoadMatch names the shape.
     * @param mixed $ctrl Optional per-call control overrides.
     * @return Distance|array The loaded Distance as an assoc-array at the
     *   SDK boundary; throws CityAutocompleteError on failure (item-5 convention).
     */
        ...
        return $this->_run_op($ctx, function () use ($ctx) {
            if ($ctx->result) {
                ...
                if ($ctx->result->resdata) {
                    $this->_data = CityAutocompleteHelpers::to_map(Struct::clone($ctx->result->resdata)) ?? [];
                }
            }
        });
    }
