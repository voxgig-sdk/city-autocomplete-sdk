...
func TestCityEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set CITYAUTOCOMPLETE_TEST_CITY_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func cityBasicSetup(extra map[string]any) *entityTestSetup {
	...
	entidEnvRaw := os.Getenv("CITYAUTOCOMPLETE_TEST_CITY_ENTID")
	...
	idmapResolved := core.ToMapAny(env["CITYAUTOCOMPLETE_TEST_CITY_ENTID"])
