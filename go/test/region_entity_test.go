...
func TestRegionEntity(t *testing.T) {
	...
	t.Run("basic", func(t *testing.T) {
		...
		if setup.syntheticOnly {
			t.Skip("live entity test uses synthetic IDs from fixture — set CITYAUTOCOMPLETE_TEST_REGION_ENTID JSON to run live")
			return
		}
		client := setup.client
...
func regionBasicSetup(extra map[string]any) *entityTestSetup {
	...
	idmap := vs.Transform(
		[]any{"region01", "region02", "region03", "country01", "country02", "country03"},
		map[string]any{
	...
	entidEnvRaw := os.Getenv("CITYAUTOCOMPLETE_TEST_REGION_ENTID")
	...
	idmapResolved := core.ToMapAny(env["CITYAUTOCOMPLETE_TEST_REGION_ENTID"])
