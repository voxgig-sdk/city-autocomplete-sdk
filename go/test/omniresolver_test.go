...
import (
	...
	omni "github.com/voxgig-sdk/city-autocomplete-sdk/go/test/omni"
...
func omniProvider(client any) *omni.Provider {
	provider := &omni.Provider{
		...
		Errify: func(err any) any {
			if sdkerr, is := err.(*sdk.CityAutocompleteError); is {
				out := map[string]any{
					"name":    "CityAutocompleteError",
...
func omniCtx(arg any, client *sdk.CityAutocompleteSDK, utility *sdk.Utility) *sdk.Context {
	...
	if provider, is := ctxmap["client"].(*omni.Provider); is {
		if live, is := defproviderclient(provider).(*sdk.CityAutocompleteSDK); is {
			client = live
