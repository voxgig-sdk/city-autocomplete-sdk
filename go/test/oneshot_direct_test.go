...
func TestOneshotDirect(t *testing.T) {
	t.Run("direct-list-oneshot", func(t *testing.T) {
		setup := oneshotDirectSetup([]any{
		...
		if _shouldSkip, _reason := isControlSkipped("direct", "direct-list-oneshot", _mode); _shouldSkip {
			if _reason == "" {
		...
		if setup.live {
			for _, _liveKey := range []string{"city_name01", "country01", "language01"} {
...
type oneshotDirectSetupResult struct {
...
func oneshotDirectSetup(mockres any) *oneshotDirectSetupResult {
	loadEnvLocal()

	calls := &[]map[string]any{}

	env := envOverride(map[string]any{
		"CITY_AUTOCOMPLETE_TEST_ONESHOT_ENTID": map[string]any{},
	...
	if live {
		...
		idmap := map[string]any{}
		if entidRaw, ok := env["CITY_AUTOCOMPLETE_TEST_ONESHOT_ENTID"]; ok {
		...
		return &oneshotDirectSetupResult{client: client, calls: calls, live: true, idmap: idmap}
	}
	...
	return &oneshotDirectSetupResult{client: client, calls: calls, live: false, idmap: map[string]any{}}
}
