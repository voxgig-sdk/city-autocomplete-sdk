...
describe('CountryTranslationDtoEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['list']) {
      if (maybeSkipControl(t, 'entityOp', 'country_translation_dto.' + op, live)) return
    }
    ...
    const country_translation_dto_ref01_list = await country_translation_dto_ref01_ent.list(country_translation_dto_ref01_match)
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['CITY_AUTOCOMPLETE_TEST_COUNTRY_TRANSLATION_DTO_ENTID']
