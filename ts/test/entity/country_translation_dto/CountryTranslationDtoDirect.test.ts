...
describe('CountryTranslationDtoDirect', async () => {
  ...
  test('direct-list-country_translation_dto', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-country_translation_dto', setup.live)) return
    if (skipIfMissingIds(t, setup, ["country_translation_dto01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['CITY_AUTOCOMPLETE_TEST_COUNTRY_TRANSLATION_DTO_ENTID']
