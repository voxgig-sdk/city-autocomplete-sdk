...
describe('OneshotEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['list']) {
      if (maybeSkipControl(t, 'entityOp', 'oneshot.' + op, live)) return
    }
    ...
    const oneshot_ref01_list = await oneshot_ref01_ent.list(oneshot_ref01_match)
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['CITY_AUTOCOMPLETE_TEST_ONESHOT_ENTID']
