...
describe('OneshotDirect', async () => {
  ...
  test('direct-list-oneshot', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-oneshot', setup.live)) return
    if (skipIfMissingIds(t, setup, ["city_name01","country01","language01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['CITY_AUTOCOMPLETE_TEST_ONESHOT_ENTID']
