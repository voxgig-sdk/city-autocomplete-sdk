...
describe('SettlementTypeDirect', async () => {
  ...
  test('direct-list-settlement_type', async (t: any) => {
    if (liveScenariosActive()) { t.skip('Covered by live operation scenarios'); return }
    const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }])
    if (maybeSkipControl(t, 'direct', 'direct-list-settlement_type', setup.live)) return
    if (skipIfMissingIds(t, setup, ["city01"])) return
...
function directSetup(mockres?: any) {
  ...
  if (live) {
    ...
    let idmap: any = env['CITY_AUTOCOMPLETE_TEST_SETTLEMENT_TYPE_ENTID']
