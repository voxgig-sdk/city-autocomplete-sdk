...
describe('SettlementTypeEntity', async () => {
  ...
  test('basic', async (t) => {
    ...
    for (const op of ['list']) {
      if (maybeSkipControl(t, 'entityOp', 'settlement_type.' + op, live)) return
    }
    ...
    if (setup.syntheticOnly) {
      t.skip('live entity test uses synthetic IDs from fixture — set CITY_AUTOCOMPLETE_TEST_SETTLEMENT_TYPE_ENTID JSON to run live')
      return
    }
    ...
    const settlement_type_ref01_list = await settlement_type_ref01_ent.list(settlement_type_ref01_match)
...
function basicSetup(extra?: any) {
  ...
  const idmapEnvVal = process.env['CITY_AUTOCOMPLETE_TEST_SETTLEMENT_TYPE_ENTID']
