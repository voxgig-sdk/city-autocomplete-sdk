...
(0, node_test_1.describe)('CityTranslationDtoDirect', async () => {
    ...
    (0, node_test_1.test)('direct-list-city_translation_dto', async (t) => {
        if (liveScenariosActive()) {
            t.skip('Covered by live operation scenarios');
            return;
        }
        const setup = directSetup([{ id: 'direct01' }, { id: 'direct02' }]);
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-list-city_translation_dto', setup.live))
            return;
        if ((0, utility_1.skipIfMissingIds)(t, setup, ["city_translation_dto01"]))
            return;
        const { client, calls } = setup;
        const params = {};
        const query = {};
        if (setup.live) {
            params.id = setup.idmap['city_translation_dto01'];
        }
        else {
            params.id = 'direct01';
        }
        const result = await client.direct({
            path: 'cities/{id}/translations',
            method: 'GET',
            params,
            query,
        });
...
function directSetup(mockres) {
    ...
    if (live) {
        ...
        let idmap = env['CITY_AUTOCOMPLETE_TEST_CITY_TRANSLATION_DTO_ENTID'];
...
//# sourceMappingURL=CityTranslationDtoDirect.test.js.map