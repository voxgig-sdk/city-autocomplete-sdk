...
(0, node_test_1.describe)('RegionDirect', async () => {
    ...
    (0, node_test_1.test)('direct-load-region', async (t) => {
        ...
        const query = {};
        if (setup.live) {
            params.id = "63df31a3-ca32-4970-8b5e-bcf9a11426e6";
        }
        else {
            params.id = 'direct01';
        }
    ...
    (0, node_test_1.test)('direct-list-region', async (t) => {
        ...
        if ((0, utility_1.maybeSkipControl)(t, 'direct', 'direct-list-region', setup.live))
            return;
        if ((0, utility_1.skipIfMissingIds)(t, setup, ["country01"]))
            return;
        const { client, calls } = setup;
        const params = {};
        const query = {};
        if (setup.live) {
            params.country_id = setup.idmap['country01'];
        }
        else {
            params.country_id = 'direct01';
        }
        const result = await client.direct({
...
function directSetup(mockres) {
    ...
    const env = (0, utility_1.envOverride)({
        'CITY_AUTOCOMPLETE_TEST_REGION_ENTID': {},
    ...
    if (live) {
        ...
        let idmap = env['CITY_AUTOCOMPLETE_TEST_REGION_ENTID'];
