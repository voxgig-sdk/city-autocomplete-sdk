...
const __1 = require("..");
const SDK_NAME = 'CityAutocompleteSDK';
...
const TEST_SEED = { "entity": { "city": { "test01": { "id": "test01" } }, "city_dto": { "test01": { "id": "test01" } }, "city_translation_dto": { "test01": { "id": "test01" } }, "country": { "test01": { "id": "test01" } }, "country_translation_dto": { "test01": { "id": "test01" } }, "distance": { "test01": { "id": "test01" } }, "language": { "test01": { "id": "test01" } }, "oneshot": { "test01": { "id": "test01" } }, "region": { "test01": { "id": "test01" } }, "region_translation_dto": { "test01": { "id": "test01" } }, "settlement_type": { "test01": { "id": "test01" } } } };
...
function isRunnable(code) {
    return (/new\s+CityAutocompleteSDK\b/.test(code) ||
        /\bCityAutocompleteSDK\.test\b/.test(code) ||
        /\bclient\s*\./.test(code));
}
...
function rewriteForRun(code) {
    ...
    out = out.replace(/new\s+CityAutocompleteSDK\s*\([^)]*\)/g, () => SEEDED_CTOR);
    out = out.replace(/CityAutocompleteSDK\.test\s*\([^)]*\)/g, () => SEEDED_CTOR);
