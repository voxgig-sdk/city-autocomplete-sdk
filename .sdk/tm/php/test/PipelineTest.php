...
// CityAutocomplete SDK pipeline test
//
...
{
    private static function utility(): CityAutocompleteUtility
    {
        return new CityAutocompleteUtility();
    }
    ...
    private static function ctx(array $over = []): CityAutocompleteContext
    {
        $utility = $over['utility'] ?? self::utility();
        $client = $over['client'] ?? new PlClient(['base' => 'http://h']);
        $ctx = new CityAutocompleteContext([
            'client' => $client,
            'utility' => $utility,
        ], null);
        $ctx->op = new CityAutocompleteOperation(['name' => 'load', 'entity' => 'x']);
        foreach ($over as $k => $v) {
    ...
    {
        return ($err instanceof CityAutocompleteError) ? $err->sdk_code : '';
    }
    ...
    public function test_make_point_rejects_a_disallowed_operation(): void
    {
        ...
        $ctx->op = new CityAutocompleteOperation(['name' => 'nope', 'entity' => 'x']);
        [$point, $err] = CityAutocompleteMakePoint::call($ctx);
    ...
    public function test_make_point_rejects_an_operation_with_no_endpoints(): void
    {
        ...
        $ctx->op = new CityAutocompleteOperation(['name' => 'load', 'entity' => 'x', 'points' => []]);
        [$point, $err] = CityAutocompleteMakePoint::call($ctx);
        $this->assertNull($point);
    ...
    public function test_make_point_returns_the_single_point(): void
    {
        ...
        $ctx = self::ctx(['options' => ['allow' => ['op' => 'load,list,create,update,remove']]]);
        $ctx->op = new CityAutocompleteOperation(['name' => 'load', 'entity' => 'x', 'points' => [$point]]);
        [$got, $err] = CityAutocompleteMakePoint::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new CityAutocompleteSpec(['method' => 'GET']);
        $ctx = self::ctx();
        $ctx->out['spec'] = $preset;
        [$got, $err] = CityAutocompleteMakeSpec::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_response_guards_missing_spec_response_result(): void
    {
        $ctx = self::ctx([
            'spec' => null,
            'response' => new CityAutocompleteResponse([]),
            'result' => new CityAutocompleteResult([]),
        ]);
        [, $err] = CityAutocompleteMakeResponse::call($ctx);
        $this->assertSame('response_no_spec', self::code($err));

        $ctx = self::ctx([
            'spec' => new CityAutocompleteSpec([]),
            'response' => null,
            'result' => new CityAutocompleteResult([]),
        ]);
        [, $err] = CityAutocompleteMakeResponse::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'spec' => new CityAutocompleteSpec(['step' => 's']),
            'response' => new CityAutocompleteResponse(self::resp(404, null, ['x-a' => '1'])),
            'result' => new CityAutocompleteResult([]),
        ]);
        [, $err] = CityAutocompleteMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_2xx_parses_the_body_and_marks_ok(): void
    {
        $ctx = self::ctx([
            'spec' => new CityAutocompleteSpec(['step' => 's']),
            'response' => new CityAutocompleteResponse(self::resp(200, ['v' => 1])),
            'result' => new CityAutocompleteResult([]),
        ]);
        [, $err] = CityAutocompleteMakeResponse::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_response_records_to_ctrl_explain_when_explain_is_on(): void
    {
        $ctx = self::ctx([
            'spec' => new CityAutocompleteSpec(['step' => 's']),
            'response' => new CityAutocompleteResponse(self::resp(200, ['v' => 2])),
            'result' => new CityAutocompleteResult([]),
        ]);
        $ctx->ctrl->explain = ['on' => true];
        CityAutocompleteMakeResponse::call($ctx);
    ...
    {
        $preset = new CityAutocompleteResponse(self::resp(299));
        $ctx = self::ctx([
            'spec' => new CityAutocompleteSpec([]),
            'response' => new CityAutocompleteResponse([]),
            'result' => new CityAutocompleteResult([]),
        ...
        [$got, $err] = CityAutocompleteMakeResponse::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null, 'result' => new CityAutocompleteResult([])]);
        [, $err] = CityAutocompleteMakeResult::call($ctx);
        $this->assertSame('result_no_spec', self::code($err));

        $ctx = self::ctx(['spec' => new CityAutocompleteSpec([]), 'result' => null]);
        [, $err] = CityAutocompleteMakeResult::call($ctx);
    ...
    public function test_make_result_list_op_wraps_resdata_into_entity_instances(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new CityAutocompleteSpec(['step' => 's']),
            'result' => new CityAutocompleteResult(['ok' => true, 'resdata' => [['a' => 1], ['a' => 2]]]),
        ]);
        $ctx->op = new CityAutocompleteOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = CityAutocompleteMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    public function test_make_result_empty_list_yields_empty_resdata(): void
    {
        ...
        $ctx = self::ctx([
            'entity' => $entity,
            'spec' => new CityAutocompleteSpec(['step' => 's']),
            'result' => new CityAutocompleteResult(['ok' => true, 'resdata' => []]),
        ]);
        $ctx->op = new CityAutocompleteOperation(['name' => 'list', 'entity' => 'x']);
        [$result, $err] = CityAutocompleteMakeResult::call($ctx);
        $this->assertNull($err);
    ...
    {
        $preset = new CityAutocompleteResult(['ok' => true]);
        $ctx = self::ctx([
            'spec' => new CityAutocompleteSpec([]),
            'result' => new CityAutocompleteResult([]),
        ]);
        $ctx->out['result'] = $preset;
        [$got, $err] = CityAutocompleteMakeResult::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    public function test_make_request_guards_a_missing_spec(): void
    {
        ...
        [, $err] = CityAutocompleteMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $boom = new CityAutocompleteError('boom', 'boom');
        $utility->fetcher = function (CityAutocompleteContext $_c, string $_u, array $_f) use ($boom): array {
            return [null, $boom];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new CityAutocompleteSpec(['step' => 's', 'method' => 'GET', 'base' => 'http://h', 'parts' => ['a']]),
        ]);
        [$response, $err] = CityAutocompleteMakeRequest::call($ctx);
        $this->assertNull($err);
    ...
    {
        $utility = self::utility();
        $utility->fetcher = function (CityAutocompleteContext $_c, string $_u, array $_f): array {
            return [null, null];
        };
        $ctx = self::ctx([
            'utility' => $utility,
    ...
    public function test_make_request_a_normal_transport_response_is_wrapped(): void
    {
        ...
        [$response, $err] = CityAutocompleteMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertInstanceOf(CityAutocompleteResponse::class, $response);
    ...
    public function test_make_request_records_the_fetchdef_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->explain = ['on' => true];
        CityAutocompleteMakeRequest::call($ctx);
    ...
    {
        $utility = self::utility();
        $utility->make_fetch_def = function (CityAutocompleteContext $c): array {
            return [null, $c->make_error('fetchdef_boom', 'boom')];
        };
        $ctx = self::ctx([
            'utility' => $utility,
            'spec' => new CityAutocompleteSpec(['step' => 's', 'method' => 'GET']),
        ]);
        [$response, $err] = CityAutocompleteMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertNotNull($response->err);
    ...
    {
        $preset = new CityAutocompleteResponse(self::resp(201));
        $ctx = self::ctx(['spec' => new CityAutocompleteSpec([])]);
        $ctx->out['request'] = $preset;
        [$got, $err] = CityAutocompleteMakeRequest::call($ctx);
        $this->assertNull($err);
        $this->assertSame($preset, $got);
    }
    ...
    {
        $ctx = self::ctx(['spec' => null]);
        [, $err] = CityAutocompleteMakeFetchDef::call($ctx);
    ...
    public function test_make_fetch_def_serialises_body_and_inits_missing_result(): void
    {
        ...
        [$fetchdef, $err] = CityAutocompleteMakeFetchDef::call($ctx);
        $this->assertNull($err);
    ...
    {
        $ctx = self::ctx(['result' => new CityAutocompleteResult(['ok' => true, 'resdata' => 42])]);
        $this->assertSame(42, CityAutocompleteDone::call($ctx));
    }

    public function test_done_raises_the_error_when_not_ok(): void
    {
        $ctx = self::ctx(['result' => new CityAutocompleteResult(['ok' => false])]);
        $this->expectException(CityAutocompleteError::class);
        CityAutocompleteDone::call($ctx);
    }

    public function test_make_error_returns_resdata_when_ctrl_throw_is_false(): void
    {
        $ctx = self::ctx(['result' => new CityAutocompleteResult(['ok' => false, 'resdata' => 'fallback'])]);
        $ctx->ctrl->throw_err = false;
        $this->assertSame('fallback', CityAutocompleteMakeError::call($ctx, null));
    }
    ...
    public function test_make_error_records_to_ctrl_explain(): void
    {
        ...
        $ctx->ctrl->throw_err = false;
        $ctx->ctrl->explain = ['on' => true];
        CityAutocompleteMakeError::call($ctx, null);
    ...
    public function test_feature_add_appends_in_call_order(): void
    {
        ...
        $a = new CityAutocompleteBaseFeature();
        $b = new CityAutocompleteBaseFeature();
        CityAutocompleteFeatureAdd::call($ctx, $a);
        CityAutocompleteFeatureAdd::call($ctx, $b);
        $this->assertSame([$a, $b], $client->features);
    }

    private static function named_feature(string $name): CityAutocompleteBaseFeature
    {
        $f = new CityAutocompleteBaseFeature();
        $f->name = $name;
        return $f;
    }
    ...
    public function test_feature_add_ordering_before_after_replace(): void
    {
        ...
        CityAutocompleteFeatureAdd::call($ctx, self::named_feature('a'));
        CityAutocompleteFeatureAdd::call($ctx, self::named_feature('b'));
        $this->assertSame(['a', 'b'], $names());

        $before = self::named_feature('z1');
        $before->_options = ['__before__' => 'b'];
        CityAutocompleteFeatureAdd::call($ctx, $before);
        $this->assertSame(['a', 'z1', 'b'], $names());

        $after = self::named_feature('z2');
        $after->_options = ['__after__' => 'a'];
        CityAutocompleteFeatureAdd::call($ctx, $after);
        $this->assertSame(['a', 'z2', 'z1', 'b'], $names());

        $replace = self::named_feature('z3');
        $replace->_options = ['__replace__' => 'z1'];
        CityAutocompleteFeatureAdd::call($ctx, $replace);
        ...
        CityAutocompleteFeatureAdd::call($ctx, $miss);
    ...
    private static function auth_ctx(array $options, ?array $headers): CityAutocompleteContext
    {
        $client = new PlClient($options);
        return self::ctx([
            'client' => $client,
            'spec' => $headers === null ? null : new CityAutocompleteSpec(['headers' => $headers]),
        ]);
    }
    ...
    public function test_prepare_auth_guards_a_missing_spec(): void
    {
        ...
        [, $err] = CityAutocompletePrepareAuth::call($ctx);
    ...
    public function test_prepare_auth_a_raw_apikey_goes_in_as_is(): void
    {
        ...
        CityAutocompletePrepareAuth::call($ctx);
    ...
    {
        $ctx = self::ctx([
            'response' => new CityAutocompleteResponse(['headers' => null]),
            'result' => new CityAutocompleteResult([]),
        ]);
        CityAutocompleteResultHeaders::call($ctx);
    ...
    public function test_result_body_skips_parsing_when_the_body_is_absent(): void
    {
        ...
        CityAutocompleteResultBody::call($ctx);
