...
class DistanceEntity
  ...
  def make
    opts = @_entopts.dup
    DistanceEntity.new(@_client, opts)
  end
  ...
  # @return [Distance, Hash] the current Distance data
  def data_get
  ...
  # @return [Hash] the current match filter (any subset of Distance fields)
  def match_get
  ...
  #
  # @param reqmatch [DistanceLoadMatch, Hash, nil] match criteria (id/query fields);
  #   optional — an entity with no id-like key loads with no match (nil is treated
  #   as an empty match, so client.Distance.load works with no args).
  # @param ctrl [Object, nil] optional per-call control
  # @return [Distance, Hash] the loaded Distance; raises CityAutocompleteError on failure
    ...
    _run_op(ctx) do
      if ctx.result
        ...
        if ctx.result.resdata
          @_data = CityAutocompleteHelpers.to_map(VoxgigStruct.clone(ctx.result.resdata)) || {}
        end
      end
    end
  end
