...
class CountryTranslationDtoEntity
  ...
  def make
    opts = @_entopts.dup
    CountryTranslationDtoEntity.new(@_client, opts)
  end

  def data_set(args)
    if args
      @_data = CityAutocompleteHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetData")
    end
  end

  # @return [CountryTranslationDto, Hash] the current CountryTranslationDto data
  def data_get
  ...
  def match_set(args)
    if args
      @_match = CityAutocompleteHelpers.to_map(VoxgigStruct.clone(args)) || {}
      @_utility.feature_hook.call(@_entctx, "SetMatch")
    end
  end

  # @return [Hash] the current match filter (any subset of CountryTranslationDto fields)
  def match_get
  ...
  #
  # @param reqmatch [CountryTranslationDtoListMatch, Hash, nil] match filter (any subset of
  #   CountryTranslationDto fields); defaults to nil, treated as an empty match that lists all.
  # @param ctrl [Object, nil] optional per-call control
  # @return [Array<CountryTranslationDto>, Array] the matching CountryTranslationDto items; raises CityAutocompleteError on failure
