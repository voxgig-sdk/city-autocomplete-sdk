...
module CityAutocompleteUtilities
  MakeRequest = ->(ctx) {
    ...
    spec = ctx.spec
    utility = ctx.utility
    response = CityAutocompleteResponse.new({})
    ...
    if fetch_err
      ...
      response = CityAutocompleteResponse.new({ "err" => ctx.make_error("request_no_response", "response: undefined") })
    elsif fetched.is_a?(Hash)
      response = CityAutocompleteResponse.new(fetched)
    else
