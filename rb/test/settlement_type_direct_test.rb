...
class SettlementTypeDirectTest < Minitest::Test
  def test_direct_list_settlement_type
    setup = settlement_type_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-settlement_type", setup[:live] ? "live" : "unit")
    if _should_skip
      skip(_reason || "skipped via sdk-test-control.json")
      return
    end
    if setup[:live]
      ["city01"].each do |_live_key|
...
def settlement_type_direct_setup(mockres)
