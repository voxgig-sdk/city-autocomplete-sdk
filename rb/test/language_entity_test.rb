...
class LanguageEntityTest < Minitest::Test
  ...
  def test_basic_flow
    ...
    if setup[:synthetic_only]
      skip "live entity test uses synthetic IDs from fixture — set CITY_AUTOCOMPLETE_TEST_LANGUAGE_ENTID JSON to run live"
      return
    end
    client = setup[:client]
    ...
    language_ref01_data_dt0_load_result = Helpers.to_map(language_ref01_data_dt0_loaded.respond_to?(:data_get) ? language_ref01_data_dt0_loaded.data_get : language_ref01_data_dt0_loaded)
    assert !language_ref01_data_dt0_load_result.nil?
    assert_equal language_ref01_data_dt0_load_result["id"], language_ref01_data["id"]
...
def language_basic_setup(extra)
  ...
  entid_env_raw = ENV["CITY_AUTOCOMPLETE_TEST_LANGUAGE_ENTID"]
