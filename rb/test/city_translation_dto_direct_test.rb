...
class CityTranslationDtoDirectTest < Minitest::Test
  def test_direct_list_city_translation_dto
    setup = city_translation_dto_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-city_translation_dto", setup[:live] ? "live" : "unit")
    if _should_skip
      skip(_reason || "skipped via sdk-test-control.json")
      return
    end
    if setup[:live]
      ["city_translation_dto01"].each do |_live_key|
...
def city_translation_dto_direct_setup(mockres)
