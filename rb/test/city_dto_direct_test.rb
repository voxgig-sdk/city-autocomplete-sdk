...
class CityDtoDirectTest < Minitest::Test
  def test_direct_list_city_dto
    setup = city_dto_direct_setup([
      { "id" => "direct01" },
      { "id" => "direct02" },
    ])
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-list-city_dto", setup[:live] ? "live" : "unit")
    if _should_skip
...
def city_dto_direct_setup(mockres)
