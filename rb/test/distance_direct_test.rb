...
class DistanceDirectTest < Minitest::Test
  def test_direct_load_distance
    setup = distance_direct_setup({ "id" => "direct01" })
    _should_skip, _reason = Runner.is_control_skipped("direct", "direct-load-distance", setup[:live] ? "live" : "unit")
    if _should_skip
...
def distance_direct_setup(mockres)
