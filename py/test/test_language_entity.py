    ...
    def test_should_run_basic_flow(self):
        ...
        if setup.get("synthetic_only"):
            pytest.skip("live entity test uses synthetic IDs from fixture — "
                        "set CITY_AUTOCOMPLETE_TEST_LANGUAGE_ENTID JSON to run live")
        client = setup["client"]
        ...
        language_ref01_data_dt0_load_result = helpers.to_map(runner.entity_data(language_ref01_data_dt0_loaded))
        assert language_ref01_data_dt0_load_result is not None
        assert language_ref01_data_dt0_load_result["id"] == language_ref01_data["id"]
...
def _language_basic_setup(extra):
    ...
    idmap_resolved = helpers.to_map(
        env.get("CITY_AUTOCOMPLETE_TEST_LANGUAGE_ENTID"))
