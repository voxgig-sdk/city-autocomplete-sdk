# CityAutocomplete SDK test runner
...
class CityAutocompleteTestRunner:
    _env = {}
    ...
    def load_env_local():
        ...
        for line in content.splitlines():
            ...
            CityAutocompleteTestRunner._env[key] = val

    @staticmethod
    def getenv(key):
        val = CityAutocompleteTestRunner._env.get(key)
        if val is not None:
            return val
    ...
    @staticmethod
    def env_override(m):
        live = CityAutocompleteTestRunner.getenv("CITY_AUTOCOMPLETE_TEST_LIVE")
        override = CityAutocompleteTestRunner.getenv("CITY_AUTOCOMPLETE_TEST_OVERRIDE")

        if live == "TRUE" or override == "TRUE":
            for key in list(m.keys()):
                envval = CityAutocompleteTestRunner.getenv(key)
        ...
        explain = CityAutocompleteTestRunner.getenv("CITY_AUTOCOMPLETE_TEST_EXPLAIN")
        if explain is not None and explain != "":
            m["CITY_AUTOCOMPLETE_TEST_EXPLAIN"] = explain
    ...
    def load_test_control():
        ...
        """
        if CityAutocompleteTestRunner._test_control is not None:
            return CityAutocompleteTestRunner._test_control
        ctrl_path = os.path.join(os.path.dirname(__file__), "sdk-test-control.json")
        try:
            with open(ctrl_path, "r") as f:
                CityAutocompleteTestRunner._test_control = json.load(f)
        except (FileNotFoundError, IOError, ValueError):
            CityAutocompleteTestRunner._test_control = {
                "version": 1,
                "test": {"skip": {
    ...
    def is_control_skipped(kind, name, mode):
        ...
        ctrl = CityAutocompleteTestRunner.load_test_control()
...
def load_env_local():
    CityAutocompleteTestRunner.load_env_local()


def env_override(m):
    return CityAutocompleteTestRunner.env_override(m)


def entity_data(v):
    return CityAutocompleteTestRunner.entity_data(v)


def entity_list_to_data(lst):
    return CityAutocompleteTestRunner.entity_list_to_data(lst)


def is_control_skipped(kind, name, mode):
    return CityAutocompleteTestRunner.is_control_skipped(kind, name, mode)


def load_test_control():
    return CityAutocompleteTestRunner.load_test_control()


def live_client_options():
    return CityAutocompleteTestRunner.live_client_options()


def live_delay_ms():
    return CityAutocompleteTestRunner.live_delay_ms()
