...
class TestDistanceEntity:

    def test_should_create_instance(self):
        testsdk = CityAutocompleteSDK.test(None, None)
        ent = testsdk.Distance(None)
        assert ent is not None

    def test_should_run_basic_flow(self):
        setup = _distance_basic_setup(None)
        # Per-op sdk-test-control.json skip — basic test exercises a flow with
        # multiple ops; skipping any one skips the whole flow (steps depend
        # on each other).
        _live = setup.get("live", False)
        for _op in ["load"]:
            _skip, _reason = runner.is_control_skipped("entityOp", "distance." + _op, "live" if _live else "unit")
            if _skip:
        ...
        distance_ref01_data = None
        if len(distance_ref01_data_raw) > 0:
            distance_ref01_data = helpers.to_map(distance_ref01_data_raw[0][1])

        # LOAD
        distance_ref01_ent = client.Distance(None)
        distance_ref01_match_dt0 = {}
        distance_ref01_data_dt0_loaded = distance_ref01_ent.load(distance_ref01_match_dt0, None)
        assert distance_ref01_data_dt0_loaded is not None



def _distance_basic_setup(extra):
    runner.load_env_local()

    entity_data_file = os.path.join(_TEST_DIR, "../../.sdk/test/entity/distance/DistanceTestData.json")
