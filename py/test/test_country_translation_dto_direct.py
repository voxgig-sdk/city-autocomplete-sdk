...
class TestCountryTranslationDtoDirect:

    def test_should_direct_list_country_translation_dto(self):
        setup = _country_translation_dto_direct_setup([
            {"id": "direct01"},
            {"id": "direct02"},
        ])
        _skip, _reason = runner.is_control_skipped("direct", "direct-list-country_translation_dto", "live" if setup["live"] else "unit")
        if _skip:
            # pytest already imported at module scope
            pytest.skip(_reason or "skipped via sdk-test-control.json")
            return
        if setup["live"]:
            for _live_key in ["country_translation_dto01"]:
...
def _country_translation_dto_direct_setup(mockres):
