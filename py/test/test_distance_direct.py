...
class TestDistanceDirect:

    def test_should_direct_load_distance(self):
        setup = _distance_direct_setup({"id": "direct01"})
        _skip, _reason = runner.is_control_skipped("direct", "direct-load-distance", "live" if setup["live"] else "unit")
        if _skip:
...
def _distance_direct_setup(mockres):
